...
void WsCheckCallBack(const std_msgs::Bool &msg) {
	if(msg.data == true) {
		marker.color.g = 1.0f;
		marker.color.r = 0.0f;
		marker.text = "Ball is Catchable";
	}
	else {
		marker.color.g = 0.0f;
		marker.color.r = 1.0f;
		marker.text = "Ball is un-attainable";
	}
...
int main( int argc, char** argv )
{
  ...
  ros::Subscriber WsCheckSub;
  ros::Subscriber configSub;
  ros::Duration(1.0).sleep();
  while (ros::ok())
    {
      WsCheckSub = n.subscribe("/WsCheck", 5, &WsCheckCallBack);
      ros::spin();
    }
}
